...
let MovieCasts = ({ getPeople, people, people_isFetching, filmography, filmography_isFetching, ...props }) => {
    ...
    return (
        <>
            ...
            <List id={null} data={filmography.crew} type={{context: 'people crew', view: 'vertical'}} />

            {/* {filmography.crew && filmography.crew.map((item, index) => <MapToCredits key={item.id} item={item} config={{
                title: 'Актеры',
                text: {
                    name: 'name',
                    job: 'character'
